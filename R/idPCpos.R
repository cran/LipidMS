...
#' FA chain at sn2) and sn2 (by default it is calculated using the difference
...
#' rules to confirm chains position. In this case, lysoPC from sn1 is at least
#' twice more intense than lysoPC from sn2.
#'
...
#'
#' idPCpos(MS1 = LipidMS::serum_pos_fullMS, MSMS1 = LipidMS::serum_pos_Ce20,
#' MSMS2 = LipidMS::serum_pos_Ce40)
#'
#' @author M Isabel Alcoriza-Balaguer <maialba@alumni.uv.es>
idPCpos <- function(MS1, MSMS1, MSMS2 = data.frame(), ppm_precursor = 5,
  ...
  if (nrow(candidates) > 0){
    # in case of PC in positive M+Na adducts identified as M+H adducts need to be
    # remove to avoid false positives
    candidates <- removeAdducts(candidates, 21.9814, ppm_precursor, rttol)
