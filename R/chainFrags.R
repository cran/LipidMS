...
#' "NL-" prefix, followed by the database and the adduct. If this neutral loss
...
#' carbons and double bounds and the ones from the precursor. On the
#' other hand, "NL-mg_M+H-H2O" will look for fragments coming from the loss of
#' MGs.
#'
...
#' \code{chainfrags} argument must be an empty vector and candidates data frame
#' and chain fragments list must be provided.
#'
...
chainFrags = function (coelfrags, chainfrags, ppm = 10, candidates = NULL,
                       f = NULL, dbs)
{
  adductsTable <- dbs[["adductsTable"]]
  if (length(chainfrags) == 0) {
    sn <- list()
    for (c in 1:nrow(candidates)) {
      if (nrow(f[[c]]) > 0) {
        diffs <- diffcb(candidates$cb[c], f[[c]]$cb,
                                  dbs[["fadb"]])
        sn[[c]] <- data.frame(cb = diffs, m.z = 0, RT = 0,
                              int = 0, db = "cbdiff", adduct = "",
                              stringsAsFactors = F)
        sn[[c]] <- sn[[c]][sn[[c]]$cb != "", ]
      } else {
        sn[[c]] <- data.frame()
      ...
      for (i in 1:length(chainfrags)) {
        if (grepl("cbdiff-", chainfrags[i])) {
          ...
          mdiff <- adductsTable[adductsTable$adduct ==
                                  unlist(strsplit(rule, "_"))[2], "mdiff"]
          fmatch <- frags(coelfrags[[c]], ppm = ppm,
                                    db, mdiff)
          if (nrow(fmatch) > 0) {
            fragments <-
              rbind(fragments,
          ...
          fmatch <- frags(coelfrags[[c]], ppm, db,
                                    mdiff)
          if (nrow(fmatch) > 0) {
    ...
    if (any(grepl("cbdiff", chainfrags))) {
      ...
      for (c in 1:nrow(candidates)) {
        if (nrow(matches[[c]]) > 0) {
          ...
          db <- diffcb(candidates$cb[c], fragments$cb,
                                 get("fadb", .GlobalEnv))
          fragments$cb <- db
