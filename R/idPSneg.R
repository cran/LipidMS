...
#'
#' PS identification based on fragmentation patterns for LC-MS/MS
...
#' here.
#'
#' @return list with PS annotations (results) and some additional information
...
#' the head group) and sn2 (FA chain as M-H). 4) Look for possible chains
#' structure based on the combination of chain fragments. 5) Check intensity
#' rules to confirm chains position. In this case, lysoPA from sn1 is at least
#' 3 times more intense than lysoPA from sn2.
#'
...
#'
#' @examples
#' idPSneg(MS1 = LipidMS::mix_neg_fullMS, MSMS1 = LipidMS::mix_neg_Ce20,
#' MSMS2 = LipidMS::mix_neg_Ce40)
#'
#' idPSneg(MS1 = LipidMS::serum_neg_fullMS, MSMS1 = LipidMS::serum_neg_Ce20,
#' MSMS2 = LipidMS::serum_neg_Ce40)
#'
#' @author M Isabel Alcoriza-Balaguer <maialba@alumni.uv.es>
idPSneg <- function(MS1, MSMS1, MSMS2 = data.frame(), ppm_precursor = 5,
                    ...
                    clrequired = c(F),
                    ftype = c("NL"),
                    chainfrags_sn1 = c("lysopa_M-H-H2O", "lysopa_M-H"),
                    chainfrags_sn2 = c("fa_M-H"),
                    intrules = c("lysopa_sn1/lysopa_sn1"),
                    rates = c("3/1"),
                    intrequired = c(T),
                    dbs = list(psdb = LipidMS::psdb,
  ...
  # candidates search
  candidates <- findCandidates(MS1, dbs[["psdb"]], ppm_precursor, rt, adducts,
                               rttol, dbs)
